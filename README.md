...
forward diagnostics 里的 forward energy / KV influence、runtime token uncertainty、导入 KV 和导出 KV，同时 12 个设备 profile 都真实触发递归长上下文调度，并且 auto-replay 至少产生一次 router threshold 和 hierarchy weight 的实际位移：
