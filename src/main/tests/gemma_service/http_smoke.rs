...
fn assert_runtime_kv_diagnostics_response_fields(body: &str) {
    for field in [
...
fn model_service_openai_models_reports_capabilities() {
    ...
    assert!(
        chat_contract_body.contains("\"stream_response_fields\"")
            ...
            && chat_contract_body.contains("\"norion.compute_budget\",\"norion.compute_budget_summary\",\"norion.compute_budget_saved_tokens\",\"norion.compute_budget_avoided_tokens\",\"norion.compute_budget_kv_lookups_skipped\",\"norion.compute_budget_fanout_reduction\",\"norion.compute_budget_read_only\",\"norion.compute_budget_write_allowed\",\"norion.compute_budget_applied\",\"norion.dna_closed_loop\",\"norion.dna_closed_loop.selected_gene_ids\",\"norion.dna_closed_loop.recursive_wave\",\"norion.dna_closed_loop.recursive_wave.planned_wave_count\",\"norion.dna_closed_loop.recursive_wave.dispatched_wave_count\",\"norion.dna_closed_loop.recursive_wave.parallel_wave_count\",\"norion.dna_closed_loop.recursive_wave.max_dispatch_width\",\"norion.dna_closed_loop.recursive_wave.runtime_calls\",\"norion.stream_state\"")
            ...
            && chat_contract_body.contains("\"norion.used_memory_count\",\"norion.used_memory_ids\",\"norion.reflection_issue_codes\",\"norion.revision_actions\",\"norion.stored_runtime_kv_memory_ids\",\"norion.route_threshold\",\"norion.route_attention_tokens\",\"norion.route_fast_tokens\",\"norion.route_attention_fraction\"")
...
fn model_service_generation_runtime_errors_return_structured_json() {
    ...
    assert!(
        openai_stream.contains("\"stored_runtime_kv_memory_ids\":[]"),
        "{openai_stream}"
    );
...
fn model_service_experience_retrieval_previews_matches_without_generation() {
    ...
    assert!(
        body.contains("\"development_evidence_surface_blocked_candidates\":0"),
        "{body}"
    );
    assert!(
