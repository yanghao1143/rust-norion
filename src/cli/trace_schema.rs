...
mod tests {
    ...
    fn runtime_closed_loop_summary_line_groups_trace_gate_counters() {
        ...
        assert!(line.contains("auto_replay_live_memory_feedback_items=1"));
        assert!(line.contains("auto_replay_live_memory_feedback_strength_delta_milli=250"));
        assert!(line.contains("auto_replay_recursive_runtime_calls=2"));
        assert!(line.contains("auto_replay_avg_recursive_call_pressure_milli=500"));
        assert!(line.contains("auto_replay_runtime_kv_budget_pressure_items=1"));
        assert!(line.contains("auto_replay_max_runtime_kv_budget_pressure_milli=800"));
        assert!(line.contains("auto_replay_runtime_kv_weak_import_pressure_items=1"));
        assert!(line.contains("auto_replay_max_runtime_kv_weak_import_pressure_milli=600"));
