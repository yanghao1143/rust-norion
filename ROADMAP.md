...
- Cargo package version is tracked by `VERSION_LEDGER.md`; `0.1.0` is retired
  as the old scaffold package version. Roadmap labels such as v0.7 and v1.0
  describe capability milestones, not package versions.
...
- The CLI can inspect memory count, experience count, adaptive global/profile
  ...
  self-evolution ledger counts, minimum reflection issue / critical issue /
  revision-action experience coverage, minimum router-threshold/
...
- State inspection now gates the same runtime token uncertainty after reload
  ...
  `--inspect-min-runtime-uncertainty-device-profiles`, and
  `--inspect-min-runtime-uncertainty-token-device-profiles`, so CI can prove
  both uncertainty-bearing experience rows and actual entropy/logprob token
  evidence reached durable experience memory rather than only benchmark
  summaries or trace lines.
