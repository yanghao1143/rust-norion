...
fn issue30_evidence_packet_cli_keeps_trace_gate_command_and_redacts_payload() {
    ...
    let output = run_cli(&[
        ...
        "--require",
        "issue375_reasoning_frame_routing_bias=general:low:520:1",
        "--require",
        "issue375_reasoning_frame_memory_policy=digest_only_profile_memory:semantic-gist-experience:4",
        "--require",
        "issue375_reasoning_frame_mutation_previews=0",
        "--require",
        ...
        "--require",
        "issue501_loop_risk_signal_count=4",
        "--require",
        ...
        "--require",
        "issue502_signal_count=3",
        "--require",
        "issue502_ranked_action_count=3",
        "--require",
    ...
    assert!(out.contains("memory_admission_gene_segment_metadata=1"));
    ...
    assert!(out.contains("issue2_memory_gene_segment_metadata_proof=true"));
    assert!(
        out.contains("issue2_memory_gene_segment_metadata_proof_source=trace_report_input_derived")
    );
    ...
    assert!(out.contains("issue375_reasoning_frame_routing_bias=general:low:520:1"));
    assert!(out.contains(
        "issue375_reasoning_frame_memory_policy=digest_only_profile_memory:semantic-gist-experience:4"
    ));
    assert!(out.contains("issue375_reasoning_frame_mutation_previews=0"));
    assert!(out.contains(
        "issue375_reasoning_frame_suppressed_capabilities=write_shell_browser_network_process_file_write_memory_write_genome_write_issue_pr_write_runtime_write"
    ));
    ...
    assert!(out.contains("issue4_dna_candidate_ledger_packet_proof=true"));
    assert!(out.contains("issue4_dna_candidate_ledger_records=1"));
    assert!(out.contains("issue4_dna_candidate_ledger_candidate_count=1"));
    assert!(out.contains("issue4_dna_candidate_ledger_candidate_only=true"));
    assert!(out.contains("issue4_dna_candidate_ledger_digest=redaction-digest:"));
    assert!(out.contains("issue4_dna_candidate_ledger_write_allowed=false"));
    assert!(out.contains("issue4_dna_candidate_ledger_applied=false"));
    assert!(out.contains("issue243_control_expression_gate_ready=true"));
    assert!(out.contains(
        "issue243_active_control_knobs=routing|context_anchor|suppression|checkpoint|memory_maintenance"
    ));
    assert!(out.contains("issue243_write_allowed=false"));
    assert!(out.contains("issue243_applied=false"));
    assert!(out.contains("issue243_operator_approval_required=true"));
    ...
    assert!(out.contains("issue501_telomere_state_present=true"));
    assert!(out.contains("issue501_remaining_tokens=0"));
    assert!(out.contains("issue501_remaining_steps=0"));
    assert!(out.contains("issue501_remaining_messages=0"));
    assert!(out.contains("issue501_repair_streak_count=2"));
    assert!(out.contains("issue501_loop_risk_signal_count=4"));
    assert!(out.contains("issue501_senescent=true"));
    assert!(out.contains("issue501_apoptosis_required=true"));
    assert!(out.contains("issue501_new_external_call_allowed=false"));
    assert!(out.contains("issue501_new_file_write_allowed=false"));
    assert!(out.contains("issue501_new_memory_write_allowed=false"));
    assert!(out.contains("issue501_new_adaptive_state_write_allowed=false"));
    assert!(out.contains("issue501_memory_promotion_allowed=false"));
    assert!(out.contains("issue501_genome_mutation_allowed=false"));
    assert!(out.contains("issue501_takeover_packet_digest=redaction-digest:"));
    assert!(out.contains("issue501_rollback_anchor_digest=redaction-digest:"));
    assert!(out.contains("issue501_handoff_next_owner=scheduler"));
    assert!(out.contains("issue501_raw_payload_present=false"));
    assert!(out.contains("issue501_preview_side_effect_allowed=false"));
    assert!(out.contains("issue502_pheromone_blackboard_present=true"));
    assert!(out.contains("issue502_signal_count=3"));
    assert!(out.contains("issue502_ranked_action_count=3"));
    assert!(out.contains("issue502_top_signal_kind=repair_first"));
    assert!(out.contains("issue502_top_action=repair_review"));
    assert!(out.contains("issue502_blackboard_digest=redaction-digest:"));
    assert!(out.contains("issue502_source_digest=redaction-digest:"));
    assert!(out.contains("issue502_payload_digest=redaction-digest:"));
    assert!(out.contains("issue502_raw_payload_present=false"));
    assert!(out.contains("issue502_side_effect_allowed=false"));
    assert!(out.contains("issue502_ttl_decay_present=true"));
    assert!(out.contains("issue502_conflict_routes_to_repair=true"));
    assert!(out.contains("issue502_ranked_actions_from_state_only=true"));
    assert!(out.contains("issue509_quorum_sensing_present=true"));
    assert!(out.contains("issue509_decision_id=redaction-digest:"));
    assert!(out.contains("issue509_quorum_report_digest=redaction-digest:"));
    assert!(out.contains("issue509_risk_class=irreversible"));
    assert!(out.contains("issue509_required_quorum_milli=700"));
    assert!(out.contains("issue509_evaluator_count=3"));
    assert!(out.contains("issue509_independent_model_count=3"));
    assert!(out.contains("issue509_independent_lane_count=3"));
    assert!(out.contains("issue509_approve_signal_count=2"));
    assert!(out.contains("issue509_reject_signal_count=1"));
    assert!(out.contains("issue509_abstain_signal_count=0"));
    assert!(out.contains("issue509_approval_concentration_milli=666"));
    assert!(out.contains("issue509_conflict_count=1"));
    assert!(out.contains("issue509_quorum_reached=false"));
    assert!(out.contains("issue509_apply_allowed=false"));
    assert!(out.contains("issue509_raw_evaluator_payload_present=false"));
    assert!(out.contains("issue509_duplicate_sources_count_once=true"));
    assert!(out.contains("issue509_conflict_routes_to_repair=true"));
    assert!(out.contains("issue509_writer_gate_bypass_allowed=false"));
    ...
    assert!(out.contains("issue377_problem_finding_kind=wasted_compute"));
    assert!(out.contains("issue377_problem_finding_severity=medium"));
    assert!(out.contains("issue377_problem_finding_confidence_milli=850"));
    assert!(out.contains("issue377_problem_finding_evidence_digest=redaction-digest:"));
    assert!(out.contains("issue377_problem_finding_source_digest=redaction-digest:"));
    assert!(out.contains("issue377_problem_finding_affected_surface=runtime_kv_reuse"));
    assert!(out.contains("issue377_problem_finding_next_step=experiment"));
    assert!(out.contains("issue377_problem_finding_raw_payload_present=false"));
    assert!(out.contains("issue377_self_observation_present=true"));
    assert!(out.contains("issue377_self_observation_id=redaction-digest:"));
    assert!(out.contains("issue377_self_observation_schema=self_observation_v1"));
    assert!(out.contains("issue377_self_observation_signal_source=runtime_trace_metrics"));
    assert!(out.contains("issue377_self_observation_source_digest=redaction-digest:"));
    assert!(out.contains("issue377_self_observation_window=second_task_roundtrip"));
    assert!(out.contains("issue377_self_observation_current_truth_digest=redaction-digest:"));
    assert!(out.contains("issue377_self_observation_digest_only=true"));
    assert!(out.contains("issue377_self_observation_raw_payload_present=false"));
    assert!(out.contains("issue377_self_observation_write_allowed=false"));
    assert!(out.contains("issue377_self_observation_applied=false"));
    assert!(out.contains("issue377_self_model_present=true"));
    assert!(out.contains("issue377_self_model_schema=control_plane_self_model_v1"));
    assert!(out.contains("issue377_self_model_scope=auditable_control_plane"));
    assert!(out.contains("issue377_self_model_claims_consciousness=false"));
    assert!(out.contains("issue377_self_model_digest_only=true"));
    assert!(out.contains("issue377_self_model_raw_payload_present=false"));
    assert!(out.contains("issue377_self_model_write_allowed=false"));
    assert!(out.contains("issue377_self_model_applied=false"));
    ...
    assert!(out.contains("issue377_hypothesis_candidate_kind=gene"));
    assert!(out.contains("issue377_hypothesis_candidate_status=promoted_for_approval"));
    assert!(out.contains("issue377_hypothesis_candidate_target_surface=reasoning_gene"));
    assert!(out.contains("issue377_hypothesis_candidate_expected_metric=memory_reuse"));
    assert!(out.contains("issue377_hypothesis_candidate_expected_direction=increase"));
    assert!(out.contains(
        "issue377_hypothesis_candidate_required_gates=trace_schema_gate|focused_tests|benchmark_gate"
    ));
    assert!(out.contains("issue377_hypothesis_candidate_rollback_anchor=redaction-digest:"));
    assert!(out.contains("issue377_hypothesis_candidate_raw_payload_present=false"));
    assert!(out.contains("issue377_hypothesis_candidate_write_allowed=false"));
    assert!(out.contains("issue377_hypothesis_candidate_applied=false"));
    assert!(out.contains("issue377_hypothesis_candidate_operator_approval_required=true"));
    ...
    assert!(out.contains("issue377_lexicographic_admission_present=true"));
    assert!(out.contains("issue377_lexicographic_admission_order=user_intent_preservation>safety>digest_only_evidence>rollback_anchor>quality_delta>cost_delta>latency_delta"));
    assert!(out.contains("issue377_user_intent_preserved=true"));
    assert!(out.contains("issue377_safety_gate_passed=true"));
    assert!(out.contains("issue377_digest_only_evidence_gate_passed=true"));
    assert!(out.contains("issue377_rollback_anchor_gate_passed=true"));
    assert!(out.contains("issue377_quality_delta_milli=125"));
    assert!(out.contains("issue377_cost_delta_milli=-80"));
    assert!(out.contains("issue377_latency_delta_milli=-35"));
    assert!(out.contains("issue377_performance_tiebreaker_only=true"));
    assert!(out.contains("issue377_hard_gate_failure_action=hold"));
    assert!(out.contains("issue377_risk_override_action=hold"));
    assert!(out.contains("issue377_negative_evidence_count=0"));
    assert!(out.contains("issue377_privacy_risk=low"));
    assert!(out.contains("issue377_license_risk=low"));
    assert!(out.contains("issue377_unsupported_capability_requested=false"));
    assert!(out.contains("issue377_unsafe_side_effect_allowed=false"));
    assert!(out.contains("issue377_risk_override_clear=true"));
    assert!(out.contains("issue377_lexicographic_admission_apply_allowed=false"));
    assert!(out.contains("issue377_best_next_state=problem_finding_preview"));
    assert!(out.contains("issue377_best_next_state_id=redaction-digest:"));
    assert!(out.contains("issue377_best_next_state_selected=true"));
    ...
    assert!(out.contains("issue377_experiment_plan_present=true"));
    assert!(out.contains("issue377_experiment_plan_id=redaction-digest:"));
    assert!(out.contains("issue377_experiment_plan_mode=preview_only"));
    assert!(out.contains(
        "issue377_experiment_plan_level_path=L0_schema_safety|L1_focused_validation|L3_benchmark"
    ));
    assert!(out.contains(
    ...
    assert!(out.contains("issue377_validation_skipped_reason=minimal_existing_evidence_path"));
    assert!(out.contains("issue377_human_apply_level=L6_human_apply"));
    assert!(out.contains("issue377_human_apply_inside_engine=false"));
    assert!(out.contains("issue377_validation_level_apply_allowed=false"));
    assert!(out.contains(
        "issue377_experiment_plan_required_gates=trace_schema_gate|focused_tests|benchmark_gate"
    ));
    assert!(out.contains("issue377_experiment_plan_budget_tokens=2048"));
    assert!(out.contains("issue377_experiment_plan_stop_on_fail=true"));
    assert!(out.contains("issue377_experiment_plan_rollback_anchor=redaction-digest:"));
    assert!(out.contains("issue377_experiment_plan_raw_payload_present=false"));
    assert!(out.contains("issue377_experiment_plan_write_allowed=false"));
    assert!(out.contains("issue377_experiment_plan_applied=false"));
    assert!(out.contains("issue377_evidence_bundle_present=true"));
    assert!(out.contains("issue377_evidence_bundle_id=redaction-digest:"));
    assert!(out.contains("issue377_evidence_bundle_schema=evidence_bundle_v1"));
    assert!(out.contains("issue377_evidence_bundle_metric=memory_reuse"));
    assert!(out.contains("issue377_evidence_bundle_direction=increase"));
    assert!(out.contains("issue377_evidence_bundle_pass_count=3"));
    assert!(out.contains("issue377_evidence_bundle_fail_count=0"));
    assert!(out.contains("issue377_evidence_bundle_command_label=issue30_fresh_checkout_smoke"));
    assert!(out.contains("issue377_evidence_bundle_refs_digest_only=true"));
    assert!(out.contains("issue377_evidence_bundle_raw_payload_present=false"));
    assert!(out.contains("issue377_evidence_bundle_write_allowed=false"));
    assert!(out.contains("issue377_evidence_bundle_applied=false"));
    assert!(out.contains("issue377_experiment_decision=promote_for_approval"));
    assert!(out.contains("issue377_experiment_decision_schema=experiment_decision_v1"));
    assert!(
        out.contains("issue377_experiment_decision_reason=clean_evidence_bundle_promotes_preview")
    );
    assert!(out.contains("issue377_experiment_decision_evidence_bundle_id=redaction-digest:"));
    assert!(out.contains("issue377_experiment_decision_target=mutation_candidate_emitter"));
    assert!(out.contains("issue377_experiment_decision_manual_approval_required=true"));
    assert!(out.contains("issue377_experiment_decision_apply_allowed=false"));
    assert!(out.contains("issue377_experiment_runner_allowed=false"));
    assert!(out.contains("issue377_experiment_apply_allowed=false"));
    assert!(out.contains("issue377_mutation_candidate_emitter_present=true"));
    assert!(out.contains("issue377_mutation_candidate_emitter_id=redaction-digest:"));
    assert!(out.contains("issue377_mutation_candidate_id=redaction-digest:"));
    assert!(out.contains("issue377_mutation_candidate_evidence_digest=redaction-digest:"));
    assert!(out.contains("issue377_mutation_candidate_rollback_anchor=redaction-digest:"));
    assert!(
        out.contains("issue377_mutation_candidate_requested_write_scope=reasoning_genome_preview")
    );
    assert!(out.contains("issue377_mutation_candidate_kind=mutation_plan_preview"));
    assert!(out.contains("issue377_mutation_candidate_preview_only=true"));
    assert!(out.contains("issue377_mutation_candidate_refs_digest_only=true"));
    assert!(out.contains("issue377_mutation_candidate_writer_gate_preflight=hold"));
    assert!(out.contains("issue377_mutation_candidate_write_allowed=false"));
    ...
    assert!(out.contains("issue377_candidate_emitter_lane_coverage=reasoning_genome_preview|memory_admission_preview|routing_policy_preview|tool_policy_preview|evolution_goal_preview"));
    assert!(out.contains("issue377_candidate_emitter_kind_coverage=mutation_plan_preview|memory_admission_preview|routing_shadow_proposal|tool_policy_candidate|evolution_goal_preview"));
    assert!(out.contains("issue377_candidate_emitter_coverage_count=5"));
    assert!(out.contains("issue377_candidate_emitter_all_preview_only=true"));
    assert!(out.contains("issue377_candidate_emitter_all_write_allowed=false"));
    assert!(out.contains("issue377_candidate_emitter_all_apply_allowed=false"));
    assert!(out.contains("issue377_candidate_emitter_all_manual_review_required=true"));
    assert!(out.contains("issue377_candidate_emitter_durable_preflight_owner=unified_writer_gate"));
    assert!(out.contains("issue377_candidate_emitter_writer_gate_bypass_allowed=false"));
    assert!(out.contains("issue377_candidate_emitter_direct_durable_write_allowed=false"));
    assert!(out.contains("issue377_candidate_emitter_ready_for_explicit_apply=false"));
    assert!(out.contains("issue377_related_issue_refs=#6|#7|#74|#79|#375"));
    assert!(out.contains("issue377_related_issue_scope_map=#6:experiment_gates|#7:memory_admission_pipeline|#74:thinking_scheduler|#79:evolution_goal_queue|#375:pre_reasoning_genome_isa"));
    assert!(out.contains("issue377_related_issue_owner_scope=meta_cognitive_evolution_loop"));
    assert!(out.contains("issue377_related_issue_non_duplicate_count=5"));
    assert!(out.contains("issue377_related_issue_all_non_duplicate=true"));
    assert!(out.contains("issue377_related_issue_apply_allowed=false"));
    assert!(out.contains("issue377_clean_room_reference_mode=rust_norion_terms_only"));
    assert!(out.contains("issue377_external_code_copied=false"));
    assert!(out.contains("issue377_external_prompt_or_schema_copied=false"));
    assert!(out.contains("issue377_restricted_license_material_copied=false"));
    assert!(out.contains("issue377_license_provenance_posture=project_owned_digest_only"));
    assert!(out.contains("issue377_clean_room_apply_allowed=false"));
