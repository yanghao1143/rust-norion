...
pub fn help_text() -> String {
    [
        ...
        "                  [--history-limit N]",
        "       norion-cli evidence-packet --issue N --commit SHA --command CMD --gate passed|failed|blocked --input PATH [--output PATH]",
        "",
        "no-backend mode: prints a local protocol snapshot only; it does not start Gemma, connect to a backend, or submit prompts",
        "evidence-packet prints or writes a deterministic redacted GitHub issue comment from a local command summary",
