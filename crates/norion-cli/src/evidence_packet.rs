...
fn trace_issue185_external_agent_lifecycle_ready_fields(
    ...
    let target_project_scope = release_field(line, "external_agent_lifecycle_target_project_scope")
        .ok_or_else(|| {
            format!(
                "{}:{} missing external_agent_lifecycle_target_project_scope",
                path.display(),
                index + 1
            )
        })?;
    ...
    let project_scope_mismatch = required_trace_count(
        path,
        index,
        line,
        "external_agent_lifecycle_project_scope_mismatch",
    )?;
...
fn issue375_reasoning_frame_ready(path: &Path, line: &str) -> Result<bool, String> {
    ...
    let executed_opcodes =
        issue375_required_field(path, line, "issue375_reasoning_frame_executed_opcodes")?;
    let routing_bias =
        issue375_required_field(path, line, "issue375_reasoning_frame_routing_bias")?;
    let memory_policy =
        issue375_required_field(path, line, "issue375_reasoning_frame_memory_policy")?;
    let mutation_previews =
        issue375_required_field(path, line, "issue375_reasoning_frame_mutation_previews")?;
    ...
    if pre_reasoning_present
        && allowed_observations
            != "repo_issue_terminal_runtime_state_task_constraints_memory_state_runtime_health"
    {
        return Err(format!(
    ...
    if pre_reasoning_present
        && action_vocab
            != "observe_inspect_compare_summarize_propose_simulate_gate_verify_quarantine_rollback"
    {
        return Err(format!(
    ...
    {
        return Err(format!(
            "{} issue375 ExpressionVM did not execute the complete Genome ISA",
            path.display()
        ));
    }
    if pre_reasoning_present && (!routing_bias.contains(':') || !memory_policy.contains(':')) {
        return Err(format!(
            "{} issue375 ReasoningFrame routing or memory policy is missing",
            path.display()
        ));
    }
    if pre_reasoning_present && mutation_previews.parse::<usize>().is_err() {
        return Err(format!(
            "{} issue375 ReasoningFrame mutation preview count is invalid",
            path.display()
        ));
    }
    if pre_reasoning_present
    ...
    Ok(pre_reasoning_present
        ...
        && executed_opcodes
            == "bind_stimulus_load_gene_match_env_express_trait_set_budget_select_memory_pack_context_focus_signal_mask_signal_declare_action_vocab_suppress_capability_require_evidence_declare_gate_preview_mutation_emit_frame"
        && routing_bias.contains(':')
        && memory_policy.contains(':')
        && mutation_previews.parse::<usize>().is_ok()
        && suppressed_capabilities
            == "write_shell_browser_network_process_file_write_memory_write_genome_write_issue_pr_write_runtime_write"
...
mod tests {
    ...
    fn issue30_context_statement_derives_context_rows_and_marks_source() {
        ...
        assert!(statement.contains("issue375_reasoning_frame_routing_bias=general:low:520:1"));
        assert!(statement.contains(
            "issue375_reasoning_frame_memory_policy=digest_only_profile_memory:semantic-gist-experience:4"
        ));
        assert!(statement.contains("issue375_reasoning_frame_mutation_previews=0"));
        assert!(statement.contains(
    ...
    fn issue375_reasoning_frame_ready_rejects_raw_frame_id() {
        let line = "issue375_pre_reasoning_genome_isa_present=true issue375_reasoning_frame_id=raw-frame issue375_reasoning_frame_environment_signals_present=true issue375_reasoning_frame_allowed_observations=repo_issue_terminal_runtime_state_task_constraints_memory_state_runtime_health issue375_reasoning_frame_action_vocab=observe_inspect_compare_summarize_propose_simulate_gate_verify_quarantine_rollback issue375_reasoning_frame_executed_opcodes=bind_stimulus_load_gene_match_env_express_trait_set_budget_select_memory_pack_context_focus_signal_mask_signal_declare_action_vocab_suppress_capability_require_evidence_declare_gate_preview_mutation_emit_frame issue375_reasoning_frame_routing_bias=general:low:520:1 issue375_reasoning_frame_memory_policy=digest_only_profile_memory:semantic-gist-experience:4 issue375_reasoning_frame_mutation_previews=0 issue375_reasoning_frame_suppressed_capabilities=write_shell_browser_network_process_file_write_memory_write_genome_write_issue_pr_write_runtime_write issue375_reasoning_frame_risk_limits=preview_only_digest_only issue375_expression_vm_side_effect=read_only issue375_genome_isa_apply_allowed=false";
