...
impl OfflineReplayAggregate {
    ...
    fn from_records(policy: &str, records: &[ReplayOutcomeRecord]) -> Self {
        ...
        let quality_avg = records
            ...
            .sum::<f64>()
            / samples as f64;
        let latency_avg_ms = records.iter().map(|record| record.latency_ms).sum::<f64>()
            / samples as f64;
...
impl OfflineReplayReport {
    pub(crate) fn from_outcome_jsonl(
        ...
        for line in outcome_jsonl.lines().map(str::trim).filter(|line| !line.is_empty()) {
        ...
        let candidate =
            OfflineReplayAggregate::from_records("profile-routing", &candidate_records);
...
impl ReplayOutcomeRecord {
    fn from_json_line(line: &str) -> Option<Self> {
        ...
        let drift_detected = json_bool_field(line, "drift_detected")
            ...
            .unwrap_or_else(|| text_field_mentions_drift(line, "reward_placeholder")
